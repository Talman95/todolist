...
export const Todolist: FC<TodoListPropsType> = memo(
  ({ demo, entityStatus, ...props }: TodoListPropsType) => {
    let tasks = useAppSelector(state => state.tasks[props.todoListID]);
    const todoList = useAppSelector(
      state => state.todoLists.filter(tl => tl.id === props.todoListID)[0],
    );
    ...
    const addTaskHandler = useCallback(
      async (title: string, helper: AddItemFormHelperType) => {
        const thunk = tasksThunks.addTask({ todoId: props.todoListID, title });
        const resultAction = await dispatch(thunk);
    ...
    const tasksComponents = tasks.map(t => {
      return <Task key={t.id} todoListID={todoList.id} taskID={t.id} />;
    });
